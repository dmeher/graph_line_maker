...
- **Drawing productivity**
  ...
  - Moving layers uses grid snapping plus snap-to-layer edges/centers via `snapRectToLayerGuides` in `src/lib/editor/source-layout.ts`; holding Alt temporarily disables snapping for the active drag. Position-only source/clipart updates retain `fillRegions`; geometry or processing changes intentionally regenerate their fill topology.
- **Image processing**
...
- Auto-save has a browser-local on/off switch and interval preference stored in `localStorage`; enabling it keeps the selected interval, and existing interval-only preferences migrate on load. Online auto-save uses `saveProjectState` only, while offline auto-save writes a session draft.
...
- **Premium editor UI contract**
  ...
  - Left-panel tabs are `layers` and `library`; source images are image layers and reusable/unplaced content belongs in Library. The layer action strip appears for one or more selected layers; grouping controls stay disabled until their selection requirements are met.
