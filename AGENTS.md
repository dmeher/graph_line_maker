...
- **Vertical split rendering.** The processor clears split columns from artwork and fill/outline hit-test masks, while preview and flattened output add a final white overlay above the separately rendered grid. After masking, both gap edges are redrawn at the normal major-grid weight so the section-ending strokes remain full-width instead of losing half their centered stroke to the white mask. PDF/print intersect that same blank range and its boundary positions with each existing tile after both artwork and vector-grid drawing. This keeps the gaps blank for either grid-layer order without changing the graph dimensions, page plan, page count, or one-window print flow.
