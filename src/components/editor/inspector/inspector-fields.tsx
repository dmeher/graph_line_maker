...
export const NumberField = memo(function NumberField({
  ...
  return (
    <label className={wrapperClassName ?? "grid min-w-0 gap-1.5"}>
      <span className={hideLabel ? "sr-only" : "text-xs font-semibold text-slate-500"}>
        {label}
      </span>
      <input
        type="number"
        ...
        className={
          inputClassName ??
          "h-10 w-full min-w-0 rounded-md border border-[var(--line)] bg-white px-3 text-sm outline-none focus:border-[var(--teal)] focus:ring-2 focus:ring-teal-100 disabled:bg-slate-100 disabled:text-slate-400"
        }
      />
    </label>
  );
    ...
    <div className="min-w-0 rounded-md border border-[#d7dde5] bg-white">
      ...
      >
        ...
        <span className="ml-auto inline-flex min-w-0 items-center gap-2">
          ...
          <span className="max-w-24 truncate font-mono text-[11px] font-semibold text-[#475467]">
            {transparentSelected ? "Transparent" : value.toUpperCase()}
          </span>
          {open ? (
            <ChevronDown size={14} className="shrink-0 text-[#667085]" aria-hidden="true" />
          ) : (
            <ChevronRight size={14} className="shrink-0 text-[#667085]" aria-hidden="true" />
          )}
        </span>
      </button>
      {open ? (
...
export function InspectorSegmented({
  ...
  return (
    <div
      className="grid rounded-md border border-[#d7dde5] bg-[#f8fafc] p-0.5"
