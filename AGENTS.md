...
- `src/lib/canvas/a4-graph-preview.ts` creates the user-initiated **A4 Preview** image from the settled **artwork-only** canvas, so its repeats deliberately omit graph lines while retaining the configured paper background. It is 300-DPI portrait A4 by default (2480×3508 px), switching to landscape only above a 30 cm graph width. The central strip contains exactly six alternating normal/mirrored full-graph repeats. A user must choose one of the ten bundled `public/preview-borders/` designs before download: the five supplied raster designs plus five original monochrome Sambalpuri Bandha-inspired SVGs based on Pasapali geometry and traditional conch/wheel, floral, fish, rudraksha, and temple-spire vocabulary. Selected vertical artwork is rotated and scaled in full, without cropping, into matching 17 mm top/bottom bands. The first 5 mm of the page is outer padding; the remaining 10 mm before the existing 15 mm top-border start contains the centered, truncated project name. The graph and bands fill all other A4 space. The editor opens this picker after the export popover unmounts. Its PNG `pHYs` metadata is set to 300 DPI for physical-print compatibility. Preview is presentation-only: it changes no project setting, export asset, or persisted image.
