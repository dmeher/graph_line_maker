...
## Targeted update - 2026-08-22 (selected-source erasing, 1% canvas zoom, and linear Preview)
...
| `PERF-EXPORT-001` | Preview is lazy-loaded only on user intent and starts from the settled flattened graph canvas. It omits the first and last 1-cell horizontal padding columns before drawing ten alternating horizontal mirrors, so repeated artwork joins without empty seams. The final canvas is always 200 cells (200 cm) wide, centers the strip, and uniformly scales only a wider legacy strip to fit; it has 5-cell decorative top/bottom bands. The layout scales down before allocation to the existing 16 MP limit, avoiding a 10× canvas-memory multiplication on large graphs; the generated PNG is transient, downloads directly, and does not change persistence, upload, or a processing signature. |

Targeted validation on 2026-08-22: focused processor, source-layout, erase-geometry, layer-extras, selected-erase-target, canvas-zoom, and linear-graph-preview tests passed; `git diff --check` passed. Rendered UI validation was not run because it was not explicitly requested.

Targeted Preview update on 2026-08-23: `linear-graph-preview.test.ts` passed 5/5, covering the 200-cell width, omitted edge columns, narrow-graph fallback, and wide-strip scaling; `git diff --check` passed. Rendered UI validation was not run because it was not explicitly requested.
