...
- **Vertical graph splits:** `GraphSettings.verticalSplits` stores multiple inclusive ranges using the same numbering as outside grid labels: number 1 is the first cell after the left 1 cm gutter, and the final 1 cm gutter is unnumbered. Each range blanks those columns from the presented artwork, fill/layer hit-testing, grid lines, and column numbers while retaining the original graph width, canvas size, project, and existing PDF/print page plan. Inspector inputs accept any positive number and either order; ranges swap reversed endpoints, clamp to the currently numbered cells when added or when the graph narrows, and merge when they overlap or touch. No database migration is required because settings remain JSONB.
...
- **Vertical split rendering.** Splits are deliberately excluded from `buildProcessingSignature`: adding/removing a presentation gap must not restart vectorization, region labeling, generated topology, or layer composition. The split-specific settings/history and undo/redo paths preserve every unrelated settings reference, and the signature cache reuses its prior value when only the split array changes; do not route split edits back through full `deriveGraphSettings`. The base artwork and region map stay reusable; the SVG preview masks gaps immediately, pointer hit-testing rejects gap x-coordinates, and flattened PNG/save output is regenerated lazily from the base artwork only when needed. A4 Preview caches a transparent split-masked artwork copy. After each white gap mask, both edges are redrawn at the normal major-grid weight so the section-ending strokes remain full-width instead of losing half their centered stroke to the mask. PDF/print intersect that same blank range and its boundary positions with each existing tile after both artwork and vector-grid drawing. This keeps the gaps blank for either grid-layer order without changing graph dimensions, page planning, or the one-window print flow.
...
- Split columns omit their outside horizontal number labels. Their white per-tile overlays are presentation-only additions to the established output composition; do not turn split ranges into new PDF files, pages, projects, or canvases.
- `src/lib/canvas/a4-graph-preview.ts` creates the user-initiated **A4 Preview** image from the settled **artwork-only** canvas, so its repeats deliberately omit graph lines while using a user-selected presentation-only backdrop inside the top/bottom border frame. It is 300-DPI portrait A4 by default (2480×3508 px), switching to landscape only above a 30 cm graph width. Its logical artwork run is 80 cm wide: the stored graph's first/last 1 cm cells are blank side gutters, so it uses `ceil(80 / (graphWidthCm - 2))` whole alternating normal/mirrored repeats for graphs wider than 2 cm (for example, an 8 cm stored graph with a 6 cm design repeats fourteen times), then scales that run into the printable A4 width. Each repeat removes those blank side-gutter cells before it is scaled onto A4. The preselected `sambalpuri-spata-default` SVG uses four five-Spata groups that alternate their starting colour, producing exactly twenty globally alternating red/white and black/white 10 mm × 10 mm slots across both 10 mm horizontal borders. Each slot reserves a fixed 0.5 mm white divider, and every inner white motif uses the same square module. It precedes fifteen other bundled `public/preview-borders/` designs: five supplied raster designs, five original monochrome Sambalpuri Bandha-inspired SVGs, and five original modern SVGs (geometric, Art Deco, monoline, circuit, and organic). A user may choose another border before choosing one of sixteen light paper colors or a transparent artwork-field backdrop. Selected vertical artwork is rotated and scaled in full, without cropping, into matching 10 mm top/bottom bands. The title strip and 5 mm outer padding remain white; only the central artwork field receives the chosen shade. The remaining 10 mm before the existing 15 mm top-border start contains the centered, truncated project name. The graph and bands fill all other A4 space. The editor opens this picker after the export popover unmounts. After both choices, the client generates one exact 300-DPI PNG Blob, shows it in the dialog, and uses that same Blob for both download and a synchronously opened single-page A4 print window; preview and print URLs are released when they are no longer needed. Its PNG `pHYs` metadata is set to 300 DPI for physical-print compatibility. Preview is presentation-only: it changes no project setting, export asset, or persisted image.
...
- **Drawing productivity**
  ...
  - The `image-eraser` tool requires **exactly one selected, unlocked source**, whether visible or hidden. It appends reversible brush strokes only to that source (stored as **normalized UV coordinates** of the working canvas, radius as a fraction of canvas width) and never mutates the uploaded original. A miss, locked source, or multi-selection changes nothing. Erased pixels are transparent, so normal source overlap reveals untouched artwork below. Hidden sources remain excluded from composition/export, but their placement and working canvas stay editable; brush drafts stamp only the hidden source and deliberately leave the visible preview unchanged until it is shown again. The user-facing brush radius is owned in **graph-output pixels** and converted to the selected source's local radius before it is stored. A contextual canvas toolbar (`.editor-context-toolbar`) exposes the brush size, the brush **Shape** (`circle | square`), and a **Restore** action for that same explicit selection.
    - **A brush gesture has one draft and one commit.** Pointer samples update only refs: a cloned selected-source working canvas receives the immediate source-local stamp and `paintEraseFootprintPreview` cuts the transient canvas inside that source's transformed pristine frame only when it is visible. Pointer-up normalizes and commits one stroke/history entry, then triggers one full processing request; pointer-cancel discards the draft and redraws the last settled preview. Starting a valid erase increments the render revision and aborts the active request, so an older async result cannot repaint over the provisional cutout.
  - The **lasso** tool (`drawingTool: "lasso"`) places discrete vertices on click instead of dragging, joins them with straight edges, and encloses the region when a click lands within `LASSO_CLOSE_DISTANCE_CELLS` of the first vertex. It likewise requires one selected, unlocked source (including a hidden one) and commits one `shape: "polygon"` stroke only to it. `fill` absent erases to transparent, while `#ffffff`/`#000000` paint over the selected artwork. Vertices map through the same `graphPixelToSourcePixel` inverse the brush uses and are stored as normalized UV, so regions stay aligned across working-canvas downscales. The polygon is clipped to the selected source's local pristine bounds; Esc cancels, Backspace removes the last vertex, and switching tools abandons an in-progress region.
    - **Target resolution uses transformed content, not broad layer bounds** (`resolveLassoTargetSource`): the selected source must intersect the polygon before its source-local region is normalized and stored. Unselected overlapping sources are never modified.
  ...
  - **Erase cache identity:** `eraseStrokesSignature` serializes every normalized stroke's points, radius, shape, and fill. It remains in `sourceProcessingCacheKey` so placed masks are rebuilt, but is intentionally absent from `sourceVectorizerCacheKey`: the base asset/background trace is reused, cloned, and locally erased before its masks are regenerated.
...
- **Image processing**
  ...
  - **Working-canvas derivation:** `sourceCanvasesRef` holds pristine loaded canvases; `ensureWorkingSourceCanvas` derives a per-source *working* canvas = pristine → background removal → erase strokes, cached in `sourceWorkingCanvasesRef` by its erase+background signature. During a brush it is cloned once and stamped incrementally, then promoted on commit. `ensureSourceTraceCanvas` separately caches pristine → background removal as the vector-trace input. `sourceProcessingCacheKey` includes erase geometry; `sourceVectorizerCacheKey` includes the source asset and background removal only. Crop still operates on the pristine canvas.
  ...
  - **Source asset sharing:** `sourceCanvasesRef` maps every layer ID to a pristine source canvas, but duplicate source paths resolve to one decoded canvas and one object URL. `sourceVectorizerCacheKey` uses source asset identity plus background-removal state rather than layer ID or erase geometry, so duplicate layers and post-trace erase revisions share native vectorization work. The source-layer count divides the capability-tier image cache budget to leave room for per-layer derived canvases.
...
- **Premium editor UI contract**
  ...
  - Tiled PDF and browser-print output preserves the project's configured physical cell size exactly: a default 1 cm cell remains 1 cm on paper. It reserves a fixed **10 mm left/right safe area** on normal paper pages. When a wide graph has a second vertical row, the whole first column has a **70 mm left hallmark lane** and a 10 mm right safe area, with source tiles split only at whole-cell boundaries and right-aligned so all first-column rows align. Other columns retain 10 mm left/right safe areas. Export pages are ordered column-major (top-to-bottom within a column, then left-to-right across columns). Dotted cut guides bracket all four tile edges; they remain export-only and must not affect canvas artwork or PNG/JSON output. The Inspector's Print dimensions and editor page-break overlay read the same unscaled `createPdfExportPlan` geometry.
  - Every tiled PDF or browser Print invocation opens a required logo picker with no initial selection. `src/lib/canvas/export-logo.ts` owns exactly two bundled choices: the established `public/brand/company-hallmark.jpeg` and `public/brand/prafulka-design.jpeg`. The chosen horizontal logo is rotated 90 degrees counter-clockwise and added only to the first column's second row (`tileX: 0`, `tileY: 1`). `companyHallmarkPlacement` fits it at up to 55 mm wide (about 98 mm tall when rotated) in the dedicated 70 mm left lane and calculates a 3 mm clearance from the actual outside-number position; the first column's right side and every other page side retain the same 10 mm margin. It must remain export-only and must not affect canvas artwork, A4 Preview PNG, or PNG/JSON output.
...
- `src/lib/editor/layer-extras.test.ts`
- `src/lib/editor/selected-erase-target.test.ts`
