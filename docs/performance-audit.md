...
| `PERF-DATA-001/002/003/004/005` | Private paths are batch-signed; save+palette persistence is transactional; the browser-local auto-save switch and interval keep interval saves settings-only; dashboard summaries are returned by a bounded RPC with six palette swatches and current-page thumbnail signing; users use 25-row cursors; create/delete/duplicate paths clean up or deep-copy nested assets. |
...
| `PERF-CANVAS-003/004`, `PERF-REACT-001` | Source-position drags now render a transient source-canvas overlay and defer the exact graph render until release, so pointer movement no longer starts a full 83-layer composition every frame. The async vector path builds masks only for each source's padded placed rectangle, merges them into the authoritative graph map, and retains unchanged source masks in a 48 MiB byte-bounded LRU keyed by source processing identity. Save, enabled auto-save, and export wait for the final exact render, preserving persisted/exported correctness. |
| `PERF-REACT-004` | Selection drag now captures all selected/grouped layer start bounds and translates them in one settings update, excluding the selection itself from snap targets. Position-only source and clipart updates retain `fillRegions` instead of discarding the full fill-override map, while geometry/processing changes still invalidate it. This preserves edited fills during a move and avoids per-member state commits for grouped drags. |
...
Current interaction contract: the action strip appears for any selected layer; group/multi-select overlays use one shared bounds box, with resize controls only on a hovered single-layer boundary. The erase-cell command is removed. Background removal and image-line erasing expose their controls only in their explicit tool modes, and image erasing renders a brush ring. The selected source can apply its processing/vectorizer settings to all source and clipart layers, after which each layer remains independently editable.

Validation on 2026-07-17: `npx tsc --noEmit --pretty false --allowImportingTsExtensions`, `npm run test:unit` (101/101), and `git diff --check` passed. Rendered browser validation remains intentionally skipped unless explicitly authorized.
