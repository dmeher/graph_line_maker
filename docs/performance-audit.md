...
| `PERF-EXPORT-001` | A4 Preview is lazy-loaded only on user intent and starts from the settled artwork-only canvas, so it omits graph lines while drawing a user-selected presentation-only backdrop only inside the top/bottom border frame. After selecting one of five supplied raster borders, five original lightweight Sambalpuri Bandha-inspired SVGs, or five original modern SVGs, the picker requires one of sixteen light paper colors or a transparent artwork-field backdrop before it creates a 300-DPI A4 PNG: portrait 2480×3508 by default and landscape 3508×2480 only for graphs wider than 30 cm. Its logical artwork run is 80 cm. The stored graph's first/last 1 cm cells are blank editor gutters, so the repeat count is `ceil(80 / (graphWidthCm - 2))` for graphs wider than 2 cm (an 8 cm stored graph with a 6 cm design becomes fourteen repeats); those whole alternating normal/mirrored copies are then scaled into the printable A4 width. Every copy excludes those two gutter cells from its artwork. Selected vertical artwork is rotated and scaled in full, without cover-cropping, into mirrored 10 mm top/bottom bands. The title strip and 5 mm outer padding remain white; only the central artwork field receives the selected shade. The remaining 10 mm before the 15 mm top-border start displays a centered, fitted project name. The picker opens after its export popover unmounts. The 8.7 MP portrait canvas remains below the 16 MP limit, and a `pHYs` chunk records 300 DPI for physical-print compatibility. Once both choices are made, the client creates one transient Blob, displays that exact PNG in the dialog, and uses the same Blob for Download and a single-page A4 Print window opened directly from the click handler; preview and print URLs are released after use. No project persistence, upload, or processing signature changes. |
...
Targeted A4 Preview update on 2026-08-26: `a4-graph-preview.test.ts` covers the sixteen-option border catalogue (five supplied, the red/dark default Sambalpuri Spata strip, five Sambalpuri-inspired, five modern), sixteen light paper colors plus transparent, portrait/landscape selection, the 80 cm-derived artwork repeat count (an 8 cm stored graph with 6 cm artwork → fourteen repeats), exclusion of both 1 cm blank side-gutter cells, the 5 mm padding / 10 mm project-name line with 10 mm top/bottom border geometry, 300-DPI PNG metadata, and one-page A4 print markup. The default Spata strip has four five-slot groups whose starting colors alternate, yielding twenty globally alternating red/white and black/white 10 mm × 10 mm slots in each border; its 0.5 mm divider and every inner white motif square are fixed-size. The picker now renders its exact transient Blob before enabling Download or Print; print uses one A4-sized popup page. Targeted test (11/11) and `git diff --check` passed. Rendered UI validation was not run because it was not explicitly requested.
