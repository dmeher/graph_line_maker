...
- `src/lib/canvas/a4-graph-preview.ts` creates the user-initiated **A4 Preview** image from the settled **artwork-only** canvas, so its repeats deliberately omit graph lines while using a user-selected presentation-only backdrop inside the top/bottom border frame. It is 300-DPI portrait A4 by default (2480×3508 px), switching to landscape only above a 30 cm graph width. Its logical artwork run is 80 cm wide: the stored graph's first/last 1 cm cells are blank side gutters, so it uses `ceil(80 / (graphWidthCm - 2))` whole alternating normal/mirrored repeats for graphs wider than 2 cm (for example, an 8 cm stored graph with a 6 cm design repeats fourteen times), then scales that run into the printable A4 width. Each repeat removes those blank side-gutter cells before it is scaled onto A4. A user must choose one of the fifteen bundled `public/preview-borders/` designs before choosing one of sixteen light paper colors or a transparent artwork-field backdrop: five supplied raster designs, five original monochrome Sambalpuri Bandha-inspired SVGs, and five original modern SVGs (geometric, Art Deco, monoline, circuit, and organic). Selected vertical artwork is rotated and scaled in full, without cropping, into matching 10 mm top/bottom bands. The title strip and 5 mm outer padding remain white; only the central artwork field receives the chosen shade. The remaining 10 mm before the existing 15 mm top-border start contains the centered, truncated project name. The graph and bands fill all other A4 space. The editor opens this picker after the export popover unmounts. Its PNG `pHYs` metadata is set to 300 DPI for physical-print compatibility. Preview is presentation-only: it changes no project setting, export asset, or persisted image.
