...
- **Drawing productivity**
  ...
  - **Line/arrow drags snap to the dominant axis** (0deg or 90deg); holding **Alt** frees the angle, matching the Alt-disables-snapping convention used when moving layers. Closed kinds are unaffected. The Outline/Filled toggle is hidden for open paths (they have no interior); `shapeSupportsSides` already excluded them from the per-side checkboxes. The canvas toolbar shows the active kind as read-only text — duplicating the picker there is what let it drift out of sync with the Shape Generator.
  - **Shape kind is a single state** (`draftShapeKind`, default `line`) shared by the Shape Generator panel and the canvas drag-draw tool. They were separate states before, so picking Line in the generator left drag-draw producing rectangles while the panel showed Line selected. Choosing a kind in the generator also calls `selectEditorTool("shape")`, because otherwise the drawing tool stayed on Cell and the drag painted grid squares. `GENERATED_SHAPE_KIND_KEYS` now includes `line` and `arrow`; drag-draw keeps **signed** width/height for those two so they can be drawn in any direction, while closed kinds normalise to positive extents. Line and arrow are open paths, so `isOpenShapeKind` marks the kinds for which Filled mode and the per-side checkboxes do not apply.
