...
| `PERF-EXPORT-001` | PDF/print tiling uses async PNG blobs, sequential yielding, object URLs, and a 240-page total cap. Every output page has a fixed 10 mm right safe area. A wide graph with a second vertical row reserves a 70 mm left lane across the full first column at whole-cell boundaries; its second row holds the rotated hallmark, separated from outside row numbers by a 3 mm clearance calculated from the tile scale. Other columns retain 10 mm left/right safe areas. Pages are emitted column-major: top-to-bottom within one column, then left-to-right. |
| `PERF-DATA-001/002/003/004/005` | Read URLs are signed **locally** against the Cloudflare media gateway with no network call, and their expiry is rounded to a time bucket so repeat renders emit byte-identical, cacheable URLs; local `MEDIA_READ_MODE=direct` instead returns authenticated same-origin `/api/media/...` and `/api/media-upload/...` URLs so browser image decoding and prepared uploads are not blocked by R2 CORS; save+palette persistence is transactional and occurs only through explicit user saves; dashboard summaries are returned by a bounded RPC with six palette swatches and render a bounded WebP card thumbnail; users use 25-row cursors; create/delete/duplicate paths clean up or deep-copy nested assets and their derivatives. |
| `PERF-STORAGE-001` | Storage moved from Supabase to Cloudflare R2, whose egress is free. Production writes use presigned direct-to-R2 PUTs with content-type and byte-length bound into the signature; production reads go through the `cloudflare-media-gateway` Worker, which verifies an HMAC before any storage access and caches objects at the edge. Local direct-mode reads and prepared uploads use authenticated app proxies to avoid browser CORS failures while preserving the signed key/type/length contract. A missing object is a hard 404 - there is no Supabase fallback. |
...
| Issue IDs | Implemented evidence |
|---|---|
| `PERF-CANVAS-002/003/005` | New-project sources persist `vectorize` (default `true`; `false` skips native vectorization, thresholding, and line thinning, then composites the placed original raster without trace-derived fill regions). Direct rasters use a white-matte uncomposite that clears tiny near-white compression residue and turns pale antialiasing into translucent foreground pixels instead of opaque white halos. Both server and editor source normalization preserve an explicit `false`; the processing signature and source mask cache key include the flag, preventing a vector/raster cache mismatch. |
| `PERF-CANVAS-002/003/005` | Per-source **working canvases** (`sourceWorkingCanvasesRef`) are derived from pristine loaded canvases via `ensureWorkingSourceCanvas` (pristine → client background-removal flood-fill → erase strokes) and cached by an erase+background signature. A valid brush gesture clones and incrementally stamps only its selected source canvas in refs, then commits one normalized stroke/history entry and one settled processing request. `sourceProcessingCacheKey` retains erase geometry so placed masks are rebuilt; `sourceVectorizerCacheKey` excludes it and shares the source-asset/background trace. The processor clones that cached trace, applies erase geometry locally, and regenerates masks without another vectorizer request. |
...
| `PERF-CANVAS-002` | Source vector cache identity uses source asset path/URL plus background-removal state rather than layer ID or erase geometry. Duplicate editable layers and erase revisions share vectorization work; placement and erase geometry still invalidate the derived mask/processing cache correctly. |
...
Current interaction contract: the action strip appears for any selected layer; Layers has per-layer and Select all checkboxes for explicit batch grouping. Group/multi-select overlays use one shared bounds box, with hovered edge/corner resize controls and Inspector Width/Height fields that proportionally scale all unlocked members. Batch Inspector controls rotate around the shared center and flip the whole selected group. Padding fields are read-only placement measurements. The erase-cell command is removed. Background removal and image-line erasing expose their controls only in their explicit tool modes, and image erasing renders a brush ring. Brush, lasso, and Restore require one explicit unlocked source selection, including a hidden source; transparent erases reveal normally overlapped lower artwork and never mutate an unselected overlap. Hidden sources remain absent from composition/export while edits persist, and hidden-source brush drafts leave the visible canvas unchanged until the source is shown. Both crop surfaces offer quick left/right/top/bottom and quarter selections that split the current crop bounds, including detected artwork, and create a normal, freely adjustable crop rectangle. The selected source can apply its processing/vectorizer settings to all source and clipart layers, after which each layer remains independently editable. Source/clipart fill overrides are region-local and survive duplicate/copy-paste and geometric transforms; destructive source processing clears only its own overrides. Client/server normalization accepts the structured scoped fill IDs as well as legacy numeric IDs, so paint changes persist through the next render and explicit save. Tiled PDF/print reserves a 70 mm left hallmark lane across the first column of a multi-row wide graph; its second row holds the supplied 90-degree-counter-clockwise hallmark, separated from grid numbers by a scale-derived 3 mm clearance. Other page sides remain 1 cm. Projects save only after an explicit Save command; offline and failed saves retain a session draft.

## Targeted update - 2026-08-25 (selected-source erasing, 1% canvas zoom, and A4 Preview)

| Issue IDs | Implemented evidence |
|---|---|
| `PERF-CANVAS-003/004`, `PERF-REACT-001` | Front/back compositor controls and stack masking are removed. Sources again use the shared-mask overlap rule, so transparency from one source reveals intact artwork below. Brush and lasso resolve exactly one unlocked, explicitly selected source, including a hidden source; a miss or multi-selection does nothing, and Restore uses that same selection. Hidden sources remain omitted from composition/export, while their transformed placement and working canvas stay editable. A hidden-source brush stamps its draft but skips preview cutout so visible artwork is untouched until unhidden. A brush keeps sampled points plus a cloned working canvas in refs, aborts/increments the active render revision before preview painting, commits once on pointer-up, and discards on cancel. Base vector traces are keyed by source asset plus background removal, while erase geometry remains in placed-mask processing identity and is applied only to a cloned trace locally. Direct raster drafts stamp their cloned working canvas incrementally and promote it only on commit. No persisted schema change. |
| `PERF-REACT-001` | Editor canvas zoom now clamps at **1%–500%**. Navigator entry, command-palette actions, fit-to-view, and pinch all use the shared zoom helpers; the stage remains CSS-scaled, so the lower floor does not allocate or re-rasterize a canvas. |
| `PERF-EXPORT-001` | A4 Preview is lazy-loaded only on user intent and starts from the settled artwork-only canvas, so it omits graph lines while drawing a user-selected presentation-only backdrop only inside the top/bottom border frame. After selecting one of five supplied raster borders, five original lightweight Sambalpuri Bandha-inspired SVGs, or five original modern SVGs, the picker requires one of sixteen light paper colors or a transparent artwork-field backdrop before it creates a 300-DPI A4 PNG: portrait 2480×3508 by default and landscape 3508×2480 only for graphs wider than 30 cm. Its logical artwork run is 80 cm. The stored graph's first/last 1 cm cells are blank editor gutters, so the repeat count is `ceil(80 / (graphWidthCm - 2))` for graphs wider than 2 cm (an 8 cm stored graph with a 6 cm design becomes fourteen repeats); the presentation-only repeat choice defaults to Flip, which uses whole alternating normal/mirrored copies, while No flip repeats every original-orientation copy across the printable A4 width. Every copy excludes those two gutter cells from its artwork. During an option swap, the current Blob remains displayed until its replacement is ready, preventing the dialog from collapsing or jumping. Selected vertical artwork is rotated and scaled in full, without cover-cropping, into mirrored 10 mm top/bottom bands. The title strip and 5 mm outer padding remain white; only the central artwork field receives the selected shade. The remaining 10 mm before the 15 mm top-border start displays a centered, fitted project name. The picker opens after its export popover unmounts. The 8.7 MP portrait canvas remains below the 16 MP limit, and a `pHYs` chunk records 300 DPI for physical-print compatibility. Once the choices are ready, the client creates one transient Blob, displays that exact PNG in the dialog, and uses the same Blob for Download and a single-page A4 Print window opened directly from the click handler; preview and print URLs are released after use. No project persistence, upload, or processing signature changes. |

Targeted validation on 2026-08-25: focused processor, source-layout, erase-geometry, layer-extras, selected-erase-target, canvas-zoom, and A4-preview tests passed; `git diff --check` passed. Rendered UI validation was not run because it was not explicitly requested.

Targeted A4 Preview update on 2026-08-26: `a4-graph-preview.test.ts` covers the sixteen-option border catalogue (five supplied, the red/dark default Sambalpuri Spata strip, five Sambalpuri-inspired, five modern), sixteen light paper colors plus transparent, portrait/landscape selection, the Flip/No flip repeat directions, the 80 cm-derived artwork repeat count (an 8 cm stored graph with 6 cm artwork → fourteen repeats), exclusion of both 1 cm blank side-gutter cells, the 5 mm padding / 10 mm project-name line with 10 mm top/bottom border geometry, 300-DPI PNG metadata, and one-page A4 print markup. The default Spata strip has four five-slot groups whose starting colors alternate, yielding twenty globally alternating red/white and black/white 10 mm × 10 mm slots in each border; its 0.5 mm divider and every inner white motif square are fixed-size. The picker now renders its exact transient Blob before enabling Download or Print; print uses one A4-sized popup page. Targeted test (12/12) and `git diff --check` passed. Rendered UI validation was not run because it was not explicitly requested.
...
| `PERF-REACT-001` (desktop dock collision update) | `command-canvas.ts` now resolves the four active desktop slots through one 12px-gutter geometry model, normalizes duplicate legacy active-slot assignments, repairs restored rectangle conflicts into their semantic slots, and retains the inert v1 `left-lower` Selection payload. The left/right lanes begin at 92px/96px, and docked Scene/Focus panels span their lane down to the 12px bottom inset; free-floating panels retain saved heights. Desktop overflow menus also offer a persisted horizontal-collapse direction: Scene/Focus become 56px, full-height edge tabs whose computed width still feeds collision-safe geometry; the standard header collapse remains vertical. Floating layout state accepts optional side `anchor` and `right` offset while old x-only records remain compatible. Left slots pack and resize from their left edge; right slots pack and resize from their right edge. Drag/nudge collisions exchange occupied slots or free positions without an intermediate persisted overlap. Navigator remains non-draggable/non-resizable, but its direct geometry follows the active slot when it is displaced by a swap. The desktop CSS selector applies each model rectangle even when a restored pod still carries `data-dock="floating"`, avoiding a shared legacy origin. Focus's <=479px container layout separates the command shelf and the one scrolling detail surface into padded cards. No project state, processing signature, history entry, API payload, or canvas render path changes. |
...
Targeted validation on 2026-08-20: the Command Canvas layout-model suite passed **19/19**, covering active-slot repair, horizontal-collapse persistence, restored floating-collision recovery, 12px side packing, lifted lane origins, bottom reach, peer-safe restored widths, anchor restoration, and free collision selection in addition to the existing persistence, docking, and movement guards. Full browser-flow, lint/type, and production-build validation remain unrun.
...
| Issue IDs | Implemented evidence |
|---|---|
| `PERF-CANVAS-002/003/005` | Derived background-removal/eraser canvases retain immutable content bounds captured from their pristine decoded source whenever they are placed or vectorized. Removing edge pixels therefore cannot recrop, rescale, or shift the remaining artwork. `eraseStrokesSignature` serializes every normalized stroke point, radius, shape, and fill, so processing identities cannot collide when earlier stroke geometry changes. Brush and lasso require one selected unlocked source, visible or hidden; unselected overlaps remain untouched and transparent erases reveal their lower artwork. A brush owns one graph-output-pixel radius, inverse-maps through that selected source's transformed pristine frame, samples refs only, and stamps a cloned working canvas. It paints the visible preview only for a visible target, so a hidden target never cuts other artwork during its draft. It aborts/increments the active render revision before the draft, commits one history entry on pointer-up, and discards on cancel. The source trace cache excludes erase geometry; the processor applies committed strokes only to a cloned cached trace before deriving masks. No schema or request contract changed. |
...
| `PERF-EXPORT-001` | PDF retains its established opaque vector-stroke grid treatment. Browser print now explicitly uses opaque SVG grid strokes and exact color adjustment on its root and SVG grid, matching the PDF's strong line contrast rather than browser-economy-faded output. Wide multi-row graphs reserve a 70 mm left hallmark lane across their full first column and a 10 mm right safe area; whole-cell tiling preserves physical cell size and row alignment. The second row carries the hallmark, kept 3 mm clear of the scale-derived outside row-number position. Other columns retain fixed 10 mm left/right safe areas, emit every column top-to-bottom before advancing right, and draw dotted guides around all four tile edges. |
...
## Remediation update - 2026-09-06 (repeatable vertical graph splits)

| Issue IDs | Implemented evidence |
|---|---|
| `PERF-CANVAS-003/004` | `GraphSettings.verticalSplits` persists normalized inclusive ranges in outside-grid-number coordinates (number 1 begins after the left 1 cm gutter). Split edits use a reference-preserving settings/history fast path (including undo/redo), reuse the cached processing signature when every non-split field is unchanged, and are excluded from the processing effect's broad settings dependency. They therefore neither deep-normalize every source/shape/erase stroke nor abort/restart image decode, vectorization, region labeling, generated topology, or layer composition. The settled base artwork/region map remains reusable; preview masking and gap hit-test rejection are immediate, while a flattened PNG is regenerated lazily only for save/export. Reversed inputs are ordered and overlapping/adjacent ranges merge before presentation. |
| `PERF-REACT-002` | The Document Inspector accepts unrestricted positive start/end inputs and adds/removes multiple split ranges through the existing changed-field history path. Values clamp to the currently numbered interior cells only when added; width changes re-normalize existing ranges, while legacy projects default to an empty list. The field lives in the existing settings JSONB, so project save, duplicate, JSON export, and session drafts require no schema migration or new request. |
| `PERF-EXPORT-001` | Preview, lazily flattened PNG, tiled PDF, and browser print mask the selected columns after artwork/grid composition, omit outside column numbers inside a gap, and then redraw both section edges at the normal major-grid weight so the centered boundary stroke is not half-clipped by the mask. A4 Preview uses a cached transparent split-masked copy of the reusable artwork. PDF/print intersect gaps and boundary positions with their existing tiles; page sizing, page count, tile order, cut guides, single print window, and established PDF splitting behavior are unchanged. |

Targeted validation on 2026-09-06: `vertical-splits.test.ts` and `exports.test.ts` pass **10/10** cases covering normalization/merge/clamp, inclusive 1-based pixel geometry, graph-space hit testing, split-only history/signature fast paths, processing-pipeline bypass, lazy flattened-output refresh, full boundary-stroke redraw order, and PDF/print composition. Runtime evidence before the fix recorded split-triggered `preview-processing` at **10.55 s** and **26.05 s** for a 12-layer project. The full unit command passes **292/294**; its only failures are the unchanged `performance-limits.test.ts` expectations that a 100×125-cell graph at 40 px/cell survives the existing 16-million-pixel safety clamp (the untouched implementation returns 89×111). Static diff validation is clean. Rendered UI automation and a production build were not run because they were not explicitly authorized.

## Remediation update - 2026-09-06 (required output logo and readable hallmark)

| Issue IDs | Implemented evidence |
|---|---|
| `PERF-EXPORT-001` | PDF and browser Print now gate generation behind a two-option logo dialog whose selection is reset to `null` for every invocation; the action cannot continue until the user chooses the established hallmark or the bundled Prafulka Design logo. The selected path feeds the existing rotated, first-column/second-row hallmark placement without adding a second print window or changing PNG, JSON, A4 Preview PNG, project persistence, or image processing. |
| `PERF-EXPORT-001` | PDF and Print pass the original `cellSizeCm` directly into layout and vector stroke sizing, so a configured 1 cm cell remains exactly 1 cm on paper. Logo readability is handled independently: the first-column hallmark lane grows from 40 mm to 70 mm and the rotated logo may occupy up to 55 mm by about 98 mm without crossing the 3 mm outside-number clearance. Whole-cell tile boundaries and the 10 mm right margin remain intact. |

Targeted validation on 2026-09-06: the PDF layout, hallmark placement, and export regression suites pass **24/24** cases, including exact 1 cm physical cells, the enlarged 70 mm logo lane, both bundled logo assets, the empty initial picker selection, and required selection before generation. Static diff validation is clean. Rendered UI/print automation and a production build were not run because they were not explicitly authorized.
...
- **Evidence:** `buildProcessingSignature` and the processing effect in `src/components/editor/editor-client.tsx`. The signature serializes global outline/fill colours, fill-region overrides, processing-relevant grid/render settings, and all source/shape/clipart inputs; settled async results also gate on both the request revision and the current signature. Presentation-only vertical split ranges use their own cheap cache signature and are intentionally excluded so they cannot abort or restart the processing pipeline.
...
- **Priority / status:** P1 / Partial
- **Evidence:** Authenticated `/api/vectorize` accepts working-resolution raster input and applies native VTracer Exact, Smooth, or Clean & thin profiles. Clean & thin traces the clamped `inkThreshold - 32` darker core while preserving the requested Line adjustment. Vector SVG/image and placed-mask identities include working dimensions and the VTracer controls; direct-raster layers deliberately use a separate identity and bypass `/api/vectorize`.
- **Impact:** VTracer remains a per-source server analysis stage, while the direct-raster option avoids it entirely. The auto-sized New Project footer retains visible controls and actions on short viewports without a fixed-row clipping failure.
- **Solution:** Measure vectorized processing latency, request payload size, SVG rasterization time, cache-hit rate, and peak memory on small, typical, and near-limit projects. Keep Exact as the compatibility default and retain direct raster imports for artwork that should not be traced.
- **Risks / validation:** High-contrast line art may still need manual Ink threshold and Line adjustment refinement. Compare rendered pixels, fill-region selection, exports, cache invalidation, and direct-raster white-matte output across JPEG/PNG/SVG-derived sources and worker/main-thread/server-route fallback behavior.
...
- **Follow-up (2026-08-02):** The derivative was the wrong *shape*, which cancelled most of the win. `createThumbnailBlob(blob, 512)` fits the whole graph in a 512px box, so a 400x4080 chart became a 50x512 strip — and `.studio-project-card__media` gave the image no definite height, so `object-fit: contain` never applied and the card magnified that strip about sevenfold into a blurry middle band. Cards now use `createCardThumbnailBlob`, which crops the top to the card's 4/3 aspect before scaling (max 768px wide, never upscaling), paired with `object-fit: cover; object-position: center top` and a definite `grid-template-rows: 100%` on the media. Existing rows were backfilled from the stored processed PNGs; the ~900 KB full-PNG fallback now applies only to projects that have never been saved with a processed image.
...
## Design workspace update — 2026-08-02

### `PERF-DESIGN-001` - editable Design state and history can grow without hard document bounds

- **Priority / status:** P1 / Resolved
- **Evidence:** `src/lib/design/schema.ts`, `src/lib/design/types.ts`, and `src/components/design/design-editor-client.tsx` enforce a 24,000 px axis limit, 16 million canvas pixels, 200 nodes, 100,000 total geometry points, 4 MiB serialized JSON, and an 80-command settled history. At 90% of the point budget, complex masks/paths are flattened into a new immutable file as an undoable command. Konva is a client-only view layer; persisted state is versioned application JSON. Server saves are single-flight and scheduled once per two-minute dirty window without resetting the deadline on every edit. Only an exact submitted snapshot can advance the synced recovery copy; edits made during a request stay dirty and schedule the next window, while the lightweight IndexedDB recovery draft remains near-immediate.
- **Impact:** Malformed or very large documents are rejected before canvas allocation or persistence, history retention is bounded, and Konva's internal serialization cannot leak transient viewport or bitmap state into database payloads.
- **Solution:** Keep client/server parsing on the shared schema, normalize scale into dimensions after transforms, and add migrations before accepting any future document version.
- **Risks / validation:** Full-canvas snapshots can still approach the shared 16 MP allocation ceiling. Unit-test limits and migration, and profile a maximum-size mixed document before raising any bound.

### `PERF-DESIGN-002` - clipart extraction and image effects can monopolize the main thread

- **Priority / status:** P1 / Partial
- **Evidence:** `src/lib/design/extraction-client.ts` caps analysis at 2 MP and transfers pixels to `extraction.worker.ts`, with a bounded main-thread fallback. Candidate output is capped at 100 and confirmed uploads run in batches of 24. Konva image-mask/filter composition and final raster export remain on the main thread, bounded by the shared 16 MP output limit.
- **Impact:** Connected-component analysis no longer blocks interaction on supported browsers, but a maximum-size mask replay or publication can still produce a visible pause on browsers without a worker/offscreen export path.
- **Solution:** Keep extraction cancellation and transfer semantics; next, move mask replay, derivative encoding, and large flattened exports behind an OffscreenCanvas worker where browser codec support is reliable.
- **Risks / validation:** Canvas filter and font rendering can differ between main-thread and worker implementations. Pixel-compare representative transparency, text, masks, filters, and blend modes before switching the authoritative publisher.

### `PERF-DESIGN-003` - shared assets can leak ownership or retain mutable cross-document references

- **Priority / status:** P1 / Resolved
- **Evidence:** `src/lib/design/paths.ts`, Design file/publish routes, library remix, template duplication, and `/api/projects/[id]/library-imports` derive all R2 keys from persisted owners and copy objects into their destination owner/document/project prefixes. Browser file and publication clients invoke the matching prepared-upload cleanup endpoint after PUT or finalize failures, so retryable failures do not normally strand immutable objects. The migration adds owner/kind/update/search indexes and bounded `get_design_library_summaries` cursor results.
- **Impact:** Library deletion cannot break a Design, template, or graph project; an admin import writes under the real project owner; list response, signing work, and DOM growth remain page-bounded.
- **Solution:** Preserve the prepare/exact-length PUT/HEAD-finalize/cleanup lifecycle and immutable object IDs. Never accept client-provided owner prefixes or mutable library references in document JSON.
- **Risks / validation:** Multi-object copies can partially succeed before a database failure. Routes delete copied paths on failure; keep ownership, forged-path, partial-upload, thumbnail-failure, and post-source-deletion coverage in API integration tests.

### `PERF-DESIGN-004` - route-shell misclassification and unconstrained editor controls can collapse the Design UI

- **Priority / status:** P1 / Resolved
- **Evidence:** `src/components/layout/app-shell.tsx` recognizes only the exact two-segment `/design/[id]` route as chrome-less, so `/design` and `/design/new` retain application navigation even with trailing slashes. `src/app/globals.css` gives the editor a bounded flex viewport, shrinkable canvas/inspector columns, constrained layer names and form controls, and explicit small-screen action visibility; the mobile dock now allocates one column for each of its five actions. `src/components/design/design-canvas-stage.tsx` gives Select exclusive ownership of layer dragging/transforms and Pan exclusive ownership of viewport dragging. It also stops layer `dragend` events at the node and independently verifies the viewport as the pan target, preventing Konva bubbling from converting a layer position into a whole-canvas displacement.
- **Impact:** The workspace no longer loses its navigation chrome, inspector values no longer concatenate or push layer actions over names, conflict banners do not increase the editor beyond the viewport, and primary actions retain usable labels and hit targets across supported widths.
- **Solution:** Keep shell detection segment-based and preserve `min-width: 0`/`minmax(0, …)` constraints at every nested grid boundary. New primary navigation items must update the dock column count or switch it to a content-derived layout.
- **Risks / validation:** Static CSS and parser checks cover structure, but breakpoint behavior still needs user-authorized rendered visual validation under the repository policy.

### `PERF-PWA-005` - Design drafts must not enter the graph editor's authenticated offline navigation cache

- **Priority / status:** P1 / Resolved
- **Evidence:** `public/sw.js` v43 blocks all `/design` navigation from offline caching, while `src/lib/design/recovery.ts` stores only a user/design-keyed IndexedDB recovery record. Successful logout clears Design recovery records in addition to service-worker project data. Root service-worker registration uses an `afterInteractive` `next/script`, avoiding React's non-executing native-script warning during client rendering.
- **Impact:** Shared library and draft HTML/RSC data cannot be served as stale offline navigation to another user, while an already-open editor can preserve unsettled work through a short disconnection or crash.
- **Solution:** Keep Design online-first until an explicitly versioned, encrypted/user-isolated synchronization design exists. Treat IndexedDB as recovery only, not an authoritative or mergeable offline copy.
- **Risks / validation:** IndexedDB cleanup depends on successful logout code running. Browser validation of crash recovery, user switching, and offline navigation requires explicit authorization under the repository policy.
