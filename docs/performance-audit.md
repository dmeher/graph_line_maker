...
Targeted A4 Preview update on 2026-08-26: `a4-graph-preview.test.ts` covers the fifteen-option border catalogue (five supplied, five Sambalpuri-inspired, five modern), sixteen light paper colors plus transparent, portrait/landscape selection, the 80 cm-derived artwork repeat count (an 8 cm stored graph with 6 cm artwork → fourteen repeats), exclusion of both 1 cm blank side-gutter cells, the 5 mm padding / 10 mm project-name line with 10 mm top/bottom border geometry, and 300-DPI PNG metadata; `git diff --check` passed. Rendered UI validation was not run because it was not explicitly requested.
