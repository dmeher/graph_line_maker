...
| `PERF-EXPORT-001` | Preview is lazy-loaded only on user intent and starts from the settled flattened graph canvas. It draws ten alternating horizontal mirrors plus 5-cell decorative top/bottom bands into one centered landscape canvas. The layout scales down before allocation to the existing 16 MP limit, avoiding a 10× canvas-memory multiplication on large graphs; the generated image is transient and opens in a new tab without persistence, upload, or a processing-signature change. |
