...
| `PERF-EXPORT-001` | A4 Preview is lazy-loaded only on user intent and starts from the settled artwork-only canvas, so it omits graph lines while drawing the configured paper background itself. The picker offers five supplied raster borders, five original lightweight Sambalpuri Bandha-inspired SVGs, and five original modern SVGs before it creates a 300-DPI A4 PNG: portrait 2480×3508 by default and landscape 3508×2480 only for graphs wider than 30 cm. Exactly six alternating normal/mirrored graph copies fill the central strip; selected vertical artwork is rotated and scaled in full, without cover-cropping, into mirrored 17 mm top/bottom bands. A 5 mm outer top padding plus the remaining 10 mm before the 15 mm top-border start display a centered, fitted project name; the other outer edges retain 5 mm padding. The picker opens after its export popover unmounts. The 8.7 MP portrait canvas remains below the 16 MP limit, and a `pHYs` chunk records 300 DPI for physical-print compatibility. The generated PNG is transient, downloads directly, and does not change persistence, upload, or a processing signature. |
