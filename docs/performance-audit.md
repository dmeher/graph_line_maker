...
| `PERF-DATA-001/002/003/004/005` | Private paths are batch-signed; save+palette persistence is transactional and occurs only through explicit user saves; dashboard summaries are returned by a bounded RPC with six palette swatches and current-page thumbnail signing; users use 25-row cursors; create/delete/duplicate paths clean up or deep-copy nested assets. |
...
| Grid defaults | `DEFAULT_MAJOR_GRID_EVERY` is `1`, so new graphs emphasize every cell boundary by default. Major grid lines are 0.5 px thicker than the configured line thickness (capped at 10 px), with higher opacity to preserve a clear but restrained separator hierarchy. Existing saved major-grid intervals remain unchanged. |
...
Current interaction contract: the action strip appears for any selected layer; Layers has per-layer and Select all checkboxes for explicit batch grouping. Group/multi-select overlays use one shared bounds box, with hovered edge/corner resize controls and Inspector Width/Height fields that proportionally scale all unlocked members. Batch Inspector controls rotate around the shared center and flip the whole selected group. Padding fields are read-only placement measurements. The erase-cell command is removed. Background removal and image-line erasing expose their controls only in their explicit tool modes, and image erasing renders a brush ring. The selected source can apply its processing/vectorizer settings to all source and clipart layers, after which each layer remains independently editable. Source/clipart fill overrides are region-local and survive duplicate/copy-paste and geometric transforms; destructive source processing clears only its own overrides. Client/server normalization accepts the structured scoped fill IDs as well as legacy numeric IDs, so paint changes persist through the next render and explicit save. Tiled PDF/print page two includes the supplied company hallmark rotated 90 degrees counter-clockwise in a non-overlapping area outside the graph. Projects save only after an explicit Save command; offline and failed saves retain a session draft.

Validation on 2026-07-17: `npx tsc --noEmit --pretty false --allowImportingTsExtensions`, `npm run test:unit` (107/107, including stable fill-region identity regression coverage), and `git diff --check` passed. Rendered browser validation remains intentionally skipped unless explicitly authorized.
...
- **Priority / status:** P1 / Partial
- **Evidence:** `saveProjectSnapshot` in `src/components/editor/editor-client.tsx` calls `saveProjectState` for settings/palettes and uploads `processedCanvasRef.current` as PNG to `src/app/api/projects/[id]/processed-image/route.ts` only for explicit manual saves and user-initiated source/clipart upload saves. Offline and failed manual saves remain session drafts.
- **Impact:** Manual saves intentionally persist generated output for downstream DB/storage availability, but canvas PNG encoding plus auth/owner checks, optional removal, Storage upload, and database update still add bandwidth and operations. Removing interval persistence eliminates repeated save operations while the user is editing.
...
- **Risks / validation:** Verify manual saves update `processed_image_path`, no save occurs while the user only edits, offline saves do not attempt network writes, and failed processed uploads surface a partial-save notice without rolling back settings.
