...
- `src/lib/canvas/linear-graph-preview.ts` creates the user-initiated **Preview** image from the settled flattened graph: it omits the first and last horizontal graph cells before repeating ten copies that alternate normal/mirrored orientation, so adjacent designs join without empty seams. The final canvas is always 200 cells (200 cm) wide, with the repeated strip centered inside it; very wide legacy graphs scale uniformly to fit. Five cell (5 cm) decorative bands sit above and below. It scales before allocation to stay within `MAX_CANVAS_PIXELS`; the editor downloads the resulting PNG directly. Preview is presentation-only: it changes no project setting, export asset, or persisted image.
