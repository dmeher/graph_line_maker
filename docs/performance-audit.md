...
| Issue IDs | Implemented evidence |
|---|---|
| `PERF-CANVAS-003/004` | `GraphSettings.verticalSplits` persists normalized inclusive ranges in outside-grid-number coordinates (number 1 begins after the left 1 cm gutter), and `buildProcessingSignature` includes them so a split edit cannot reuse a stale settled frame. The compositor clears only each range from the existing artwork/fill/outline typed arrays and output canvas; reversed inputs are ordered and overlapping/adjacent ranges merge before rendering. This adds no graph-sized allocation, image decode, vectorization request, or extra full-frame scan. |
